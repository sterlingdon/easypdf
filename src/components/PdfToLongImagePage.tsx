...
export default function PdfToLongImagePage() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const imageBlob = await PdfToImage.toLongImage(file.file, format);
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.download = `${file.file.name.replace('.pdf', '')}_long_image.${format}`;
      ...
      <div className="space-y-4">
        ...
        <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl">
          <p className="text-sm text-slate-700">
            <span className="font-bold">Long Image</span> - All PDF pages will be stitched vertically into one continuous image. Perfect for sharing on social media or mobile viewing.
          </p>
        </div>
      </div>
    </>
  ...
  const successContent = (
    <div className="min-h-screen bg-[#f7f7f7] pb-20" style={{
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 text-center">
        ...
        <h1 className="text-4xl font-bold text-slate-900 mb-4">Long Image Created!</h1>
        <p className="text-slate-500 mb-12">All PDF pages have been combined into one long image.</p>
        ...
        <div className="mt-12">
          <button
            onClick={() => { setStatus('IDLE'); setFile(null); setResultImage(null); }}
  ...
  return (
    <ToolPageLayout
      ...
      description="Convert all PDF pages into a single long image stitched together."
      files={file ? [file] : []}
      status={status}
      maxFiles={1}
